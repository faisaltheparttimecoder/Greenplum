...
CREATE OR REPLACE FUNCTION check_greenplum.fn_create_db_files_skew() RETURNS void AS
$$
DECLARE 
        v_function_name text := 'fn_create_db_files_skew';
        v_location int;
        v_sql text;
        v_db_oid text;
...
BEGIN
        ...
        v_location := 2000;
        v_sql := 'DROP VIEW IF EXISTS check_greenplum.vw_file_skew';

        v_location := 2100;
        EXECUTE v_sql;
        
        v_location := 2200;
        v_sql := 'DROP EXTERNAL TABLE IF EXISTS check_greenplum.db_files_skew';

        v_location := 2300;
        EXECUTE v_sql;

        v_location := 3000;
        v_sql := 'CREATE EXTERNAL WEB TABLE check_greenplum.db_files_skew ' ||
        ...
        v_location := 4200;
        v_sql := 'CREATE OR REPLACE VIEW check_greenplum.vw_file_skew AS ' ||
                 'SELECT schema_name, ' ||
                 'table_name, ' ||
                 ...
                 '      sum(db.size) as size ' ||
                 '      FROM check_greenplum.db_files_skew db ' ||
                 '      JOIN pg_class c ON ' ||
...
SELECT check_greenplum.fn_create_db_files_skew();